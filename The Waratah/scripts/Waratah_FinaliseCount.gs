/****************************************************
 * THE WARATAH - WEEKLY COUNTS — FINALISE STOCKTAKE (INTERFACE BUTTON)
 *
...
const CONFIG = {
  ...
  // Items fields
  // NOTE: Both itemNameField and recipeProducesItemField reference "Item Name" but in different tables:
  // - In the Items table: "Item Name" is the primary text field (display name)
  // - In the Recipes table: "Item Name" is a linked-record field pointing to Items
  // Do not merge these into a single key — they serve different schema roles.
  ...
  // Script identifier
  scriptName: "WARATAH - FINALISE COUNT",
};
...
const validateRecipeIntegrity_ = async () => {
  console.log("");
...
const formatRecipeIssues_ = (issues) => {
  const parts = [];
...
const main = async () => {
  ...
  // PROLOGUE: Recipe Integrity Validation (optional, not counted as a numbered phase)
  ...
  const hasSourceField = safeField_(countsTable, CONFIG.countSourceField);
  ...
  // PHASE 4: Normalize timestamp and prepare updates
  console.log("");
  ...
  // PHASE 7: Calculate execution time
  const endTime = Date.now();
  const executionTime = ((endTime - startTime) / 1000).toFixed(2);
