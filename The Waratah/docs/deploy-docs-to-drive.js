...
const arg = process.argv[2] || "staff";
const selectedTargets =
  arg === "all" ? Object.keys(TARGETS) : [arg];

if (!selectedTargets.every((k) => TARGETS[k])) {
  console.error(`Usage: node deploy-docs-to-drive.js [staff|advanced|all]`);
  process.exit(1);
}
...
async function main() {
  ...
  for (const targetKey of selectedTargets) {
    ...
    for (const mdFile of target.files) {
      const mdPath = path.join(target.sourceDir, mdFile);
