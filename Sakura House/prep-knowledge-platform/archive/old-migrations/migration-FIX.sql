-- FIX: Updated search function with proper column references

DROP FUNCTION IF EXISTS search_with_analytics(TEXT, vector(1536), INT, FLOAT, TEXT, TEXT);
...
BEGIN
  ...
  -- Calculate stats (use alias to avoid ambiguity)
  SELECT COUNT(*), MAX(tr.sim_score), AVG(tr.sim_score)
  ...
  -- Return results
  RETURN QUERY
  SELECT tr.id, tr.content, tr.sim_score, tr.metadata, v_search_id
  FROM temp_results tr;
