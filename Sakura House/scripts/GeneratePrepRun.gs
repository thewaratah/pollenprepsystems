/****************************************************
 * PREP RUN GENERATOR — REBUILD MODE (INTERFACE BUTTON)
 * 
 * VERSION: 3.0 (with audit logging + buffer multiplier + dynamic reorder points)
 *
 * Purpose:
 ...
 *
 * CHANGES IN v3.0:
 *   - Dynamic reorder point system: items only trigger prep when stock
 *     drops below their individual reorder point (not just any shortfall)
 *   - Reorder point = (Weekly Volume (ml) × 1.05) / 1.10
 *   - Items without Weekly Volume (ml) fall back to original behaviour
 *   - Items above reorder point are skipped and logged in audit trail
 *
 * CHANGES IN v2.0:
...
const CONFIG = {
  ...
  itemExpectedSalesField: "Weekly Volume (ml)",

  // Reorder Point constants (v3.0)
  reorderWastagePct: 0.05,   // 5% wastage — items need slightly more made than sold
  reorderSafetyPct: 0.10,    // 10% safety buffer for demand uncertainty
...
function safeField_(table, fieldNameOrId) {
  try {
...
function normalizeToMinuteMs_(date) {
  return Math.floor(date.getTime() / 60000) * 60000;
}

function formatSydneyDDMMYYYYHHMM_(d) {
...
function getSingleSelectName_(record, fieldName) {
  const v = record.getCellValue(fieldName);
  return v && typeof v.name === "string" ? v.name : null;
}

function getLinkedRecordIds_(record, fieldName) {
  const v = record.getCellValue(fieldName);
  if (!v || !Array.isArray(v)) return [];
  return v.map((x) => x?.id).filter(Boolean);
}

function getLinkedRecordId_(record, fieldName) {
  const ids = getLinkedRecordIds_(record, fieldName);
  return ids.length ? ids[0] : null;
}

function coerceLookupToText_(value) {
...
async function callAirtable_(label, fn, payloadForError) {
  try {
    return await fn();
  } catch (e) {
...
function setTextOrSelectField_(fieldsObj, table, fieldName, valueText) {
...
function setSingleSupplierLinkIfPossible_(fieldsObj, table, fieldName, supplierIds) {
  const field = safeField_(table, fieldName);
  if (!field) return;
...
function buildRequestUpdateFields_(requestsTable, partial) {
  const fields = {};
...
async function safeUpdateRequest_(requestsTable, requestId, partial) {
...
async function main() {
  ...
  const itemsQuery = await itemsTable.selectRecordsAsync({
    fields: [
      ...
      CONFIG.itemBufferMultiplierField,
      CONFIG.itemExpectedSalesField,
    ],
  });
  ...
  const itemMetaById = {};
  const itemBufferById = {};
  const itemExpectedSalesById = {};
  const itemNameById = {};

  for (const rec of itemsQuery.records) {
    const itemId = rec.id;
    itemNameById[itemId] = rec.name || itemId;
    ...
    // Buffer Multiplier (default to 1.0 if blank)
    ...
    // Weekly Volume (ml) (for reorder point — null means fallback to original behaviour)
    const salesField = safeField_(itemsTable, CONFIG.itemExpectedSalesField);
    if (salesField) {
      const salesRaw = rec.getCellValue(CONFIG.itemExpectedSalesField);
      itemExpectedSalesById[itemId] = (typeof salesRaw === "number" && salesRaw > 0) ? salesRaw : null;
    } else {
      itemExpectedSalesById[itemId] = null;
    }
  }
  ...
  // Returns the stock level at which prep should trigger for this item.
  // If Weekly Volume (ml) is set: reorder point = (volume × 1.05) / 1.10, capped at par.
  // If not set: falls back to par (original behaviour — always prep on any shortfall).
  function getReorderPoint(expectedSales, parQty) {
    if (!expectedSales || expectedSales <= 0) return parQty;
    const rp = (expectedSales * (1 + CONFIG.reorderWastagePct)) / (1 + CONFIG.reorderSafetyPct);
    return Math.min(rp, parQty);
  }

  const skipped = []; // Items that had stock above their reorder point this run

  // Top-level batches
  for (const itemRec of itemsQuery.records) {
    const itemId = itemRec.id;
    if (!itemActiveById[itemId]) continue;
    if (itemTypeById[itemId] !== CONFIG.allowedTopLevelItemType) continue;

    const parQty = par[itemId] || 0;
    const currentStock = onHand[itemId] || 0;
    const reorderPoint = getReorderPoint(itemExpectedSalesById[itemId], parQty);

    if (currentStock >= reorderPoint) {
      skipped.push({ itemId, currentStock, reorderPoint, parQty });
      continue;
    }

    const shortfall = parQty - currentStock;
  ...
  if (skipped.length) {
    console.log(`  → Skipped ${skipped.length} items (stock above reorder point):`);
    skipped.forEach(s => console.log(`     • ${itemNameById[s.itemId]}: stock ${round3(s.currentStock)} >= reorder ${round3(s.reorderPoint)} (par: ${round3(s.parQty)})`));
  }
  ...
  console.log(`Items Skipped (above reorder point): ${skipped.length}`);
  ...
  const skippedLines = skipped.length
    ? skipped.slice(0, 25).map(s =>
        `  • ${itemNameById[s.itemId] || s.itemId}: stock ${round3(s.currentStock)} >= reorder ${round3(s.reorderPoint)} (par: ${round3(s.parQty)})`
  ...
  const details =
    ...
    `Top-level Batches: ${allTasks.filter(t => itemTypeById[t.itemId] === CONFIG.allowedTopLevelItemType).length}\n` +
    `Sub-recipe Tasks: ${allTasks.filter(t => CONFIG.subRecipeItemTypes.has(itemTypeById[t.itemId])).length}\n` +
    `Total Tasks: ${allTasks.length}\n` +
    `Items Skipped (above reorder point): ${skipped.length}\n${skippedLines}\n` +
  ...
  output.set("itemsSkipped", skipped.length);
