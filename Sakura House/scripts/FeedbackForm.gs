 ...
 *
 * Allows Gooch, Sabs, and prep team to submit feedback about:
...
const FEEDBACK_CONFIG = {
  ...
  docTypes: ['Ingredient Prep List', 'Batching List', 'Gooch Ordering', 'Sabs Ordering'],
  staffRoles: ['Prep Team', 'Ordering - Gooch', 'Ordering - Sabs', 'Manager', 'Other'],
...
function searchItems(query) {
  ...
  do {
    let url = `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(FEEDBACK_CONFIG.airtable.tables.items)}?fields[]=Item%20Name&pageSize=100`;
...
function searchRecipes(query) {
  ...
  do {
    let url = `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(FEEDBACK_CONFIG.airtable.tables.recipes)}?fields[]=Recipe%20Name&pageSize=100`;
    if (offset) url += `&offset=${offset}`;
    ...
    const data = JSON.parse(response.getContentText());
    allRecipes = allRecipes.concat(data.records || []);
    offset = data.offset || null;
  } while (offset);
...
function findRecipeLine_(recipeReferenceId, description) {
  ...
  try {
    // Fetch recipe lines with pagination
    let allLines = [];
    let offset = null;

    do {
      let url = `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(FEEDBACK_CONFIG.airtable.tables.recipeLines)}?fields[]=Recipe&fields[]=Item&fields[]=Qty&pageSize=100`;
      if (offset) url += `&offset=${offset}`;
      ...
      const data = JSON.parse(response.getContentText());
      allLines = allLines.concat(data.records || []);
      offset = data.offset || null;
    } while (offset);
...
function sendFeedbackSlack_(feedback, triageResult) {
  const props = PropertiesService.getScriptProperties();
  const webhookUrl = props.getProperty('SLACK_WEBHOOK_EV_TEST');

  if (!webhookUrl) {
    Logger.log('SLACK_WEBHOOK_EV_TEST not configured, skipping notification');
    return false;
  }
